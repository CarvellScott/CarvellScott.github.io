...
When I first learned of tab-completion in Bash, on my first day in a programming job, I was elated. It was absolutely wonderful to type out a few letters of some really long file in the terminal, and have the computer complete the name for me.

What was not wonderful, was when I wanted to make my own custom completion functions. This is not just because of how weird bash syntax is to me coming from a C/C++ background, but because of how tricky it was to figure out what's going on with them.

Most people write completion specifications ("comp specs") as bash functions and add them via `complete -F _completion_function cmd`. There is good reason for this, `/usr/share/bash-completion/` contains hundreds of examples to work with. For those of us who have trouble with writing stuff beyond simple pipelines/small functions in bash, we have to use `complete -C command_to_generate_completions cmd`. [The manual entry*](https://www.gnu.org/software/bash/manual/html_node/Programmable-Completion-Builtins.html) states:
> command is executed in a subshell environment, and its output is used as the possible completions

[This entry is also helpful](https://www.gnu.org/software/bash/manual/html_node/Programmable-Completion.html):
...
Summarized, I need to make a program that accepts three command-line arguments, and four environment variables, and prints stuff that matches one of the parameters to stdout. Easy enough. Here's the template:
...
As an example, here's my completion for my 'up' command, which is just an alias for `cd`:
...
if __name__ == "__main__":
    main()
```
After you install the comp spec with `complete -C name_of_wherever_you_save_the_above.py up`, this script will split a path like `/mnt/c/Users/U/Userutils/foobar` to generate completion values like "mnt", "c", "Users", "U", "UserUtils", each corresponding to the longest part of the directory that ends in it. When you type `up Users` at the terminal, `Users` will get replaced with `/mnt/c/Users`, ready as input for whatever `up` is aliased to.

While your eyes are lighting up with possibilities for completions in any language other than bash, I should mention there are a few gotchas that show up when you start writing complex completions:
...
For the most part, you can trial-and-error your way towards a robust  completion script, probably just writing variables out to a file to inspect them, but if you're like me, you feel like it'd be nice if there was a way you could simulate `complete` by calling it with the necessary parameters and environment variables programmatically... like so:
...
So to summarize:
1. Completion scripts csn be written in any language, and that's incredibly underrated.
2. It's underrated because complete doesn't lend itself well to automation, let alone debugging.
